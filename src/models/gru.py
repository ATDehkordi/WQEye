...
# ----------------------------
# Your original GRU architecture (logic preserved)
# ----------------------------
class GRURegressor(nn.Module):
    # def __init__(self, bidirectional, input_GRU_size, GRU_layers_with_hidden_sizes, FCNN_layers_with_hidden_sizes, FCNN_act, FCNN_dropout_rate):
    def __init__(
        self,
        bidirectional,
        input_GRU_size,
        GRU_layers_with_hidden_sizes,
        ...
        super().__init__()
        """
        If FCNN_layers_with_hidden_sizes is [0], no FCNN head is applied and the network passes only the last hidden
        state into a final Linear layer. 'bidirectional' must be True or False.
        """
        self.bidirectional = bidirectional
        self.input_GRU_size = input_GRU_size
        self.GRU_layers_with_hidden_sizes = GRU_layers_with_hidden_sizes

        # FCNN input size depends on bidirectional setting
        self.input_FCNN_size = (
            GRU_layers_with_hidden_sizes[-1] * 2 if self.bidirectional else GRU_layers_with_hidden_sizes[-1]
        )
        ...
        # ---- GRU stack ----
        layers_GRU = []
        in_GRU_size = self.input_GRU_size
        for hidden_size in self.GRU_layers_with_hidden_sizes:
            layers_GRU.append(
                nn.GRU(
                    input_size=in_GRU_size,
            ...
            in_GRU_size = hidden_size * 2 if self.bidirectional else hidden_size
        self.layers_GRU = nn.ModuleList(layers_GRU)
    ...
    def forward(self, x):
        ...
        for gru in self.layers_GRU:
            x, hn = gru(x)  # hn: (num_layers * num_directions, B, H)

        if self.bidirectional:
            # Concatenate last forward/backward hidden states along feature dimension
            final_fw = hn[0, :, :]  # (B, H)
            final_bw = hn[1, :, :]  # (B, H)
...
# ----------------------------
# Project-aligned wrapper model (same structure as LSTMModel)
# ----------------------------
class GRUModel(BaseMLModel):
    """GRU model aligned with the project's training/tuning/predict interfaces."""
    ...
    def _ensure_3d(self, X_df: pd.DataFrame) -> torch.Tensor:
        """
        Convert tabular input (N, T) into the expected (N, T, 1) tensor for GRU input.
    ...
    def fit(self, X_train: pd.DataFrame, y_train: pd.Series, **kwargs) -> None:
        ...
        best_csv = "outputs/GRU_bestparams.csv"
        ...
        self.model = GRURegressor(
            ...
            input_GRU_size=1,
            GRU_layers_with_hidden_sizes=params.get("GRU_layers_with_hidden_sizes", [64, 64]),
    ...
    def tune(self, X_train: pd.DataFrame, y_train: pd.Series, param_grid: dict, k_folds: int, scalers: dict, **kwargs):
        ...
        """
        hyper_csv = "outputs/GRU_hyperparametertuning.csv"
        ...
        param_grid["GRU_layers_with_hidden_sizes"] = _normalize_layers_list(
            param_grid.get("GRU_layers_with_hidden_sizes", [[64, 64], [32]])
        )
        ...
        status = st.status("Starting GRU tuning...", expanded=False)
        ...
        for i, params in enumerate(combos):
            ...
            for train_idx, val_idx in kf.split(X_train):
                ...
                model = GRURegressor(
                    ...
                    input_GRU_size=1,
                    GRU_layers_with_hidden_sizes=params.get("GRU_layers_with_hidden_sizes"),
                ...
                with torch.no_grad():
                    ...
                    # Back to the original scale (project-consistent)
        ...
        status.update(label="GRU tuning complete!", state="complete")
    ...
    def get_default_param_grid(self, X_train) -> dict:
        """
        EXACT default you requested (same as LSTM but with GRU layer key).
        """
        return {
            "bidirectional": [False],
            "GRU_layers_with_hidden_sizes": [[64, 64]],
    ...
    def get_param_definitions(self) -> dict:
        """
        EXACTLY these params are exposed 
        """
        return {
            "bidirectional": {
                ...
                "options": [True, False],
                "type": bool,
                "help": "Use bidirectional GRUs or not.",
            },
            "GRU_layers_with_hidden_sizes": {
                "label": "GRU Hidden Sizes (per layer)",
                "ui_widget": "text_list",
                "placeholder": "e.g., [64 64], [32 32], [64 32], [64], [32]",
                "type": str,
                "help": "List(s) of hidden sizes per GRU layer. Use bracketed lists separated by commas.",
            },
            "FCNN_layers_with_hidden_sizes": {
