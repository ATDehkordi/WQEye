...
class KANModel(BaseMLModel):
    ...
    def fit(self, X_train: pd.DataFrame, y_train: pd.Series, **kwargs) -> None:
        ...
        if len(default_layers)==1:
            # print([input_size, sub[0], 1])
            self.model = KAN([input_size, default_layers[0], 1], grid_size=params.get('grid_size'), spline_order=params.get('spline_order'))
            # model = KAN([input_size, hidden_layer_size[0], 1], spline_order=spline_order)
        else:
            # print([input_size, *sub, 1])
            self.model = KAN([input_size, *default_layers, 1], grid_size=params.get('grid_size'), spline_order=params.get('spline_order'))
